...
# Definitely thought this was going to throw an error.
...
# hi there because informal greeting is pointing to 'hi' object and we've mutated that object.
...
# All the same, assignment is not mutating.
...
# Reassignment is still not mutating.
...
# D) Ben was tasked to write a simple ruby method to determine if an input string is an
# IP address representing dot-separated numbers. e.g. "10.4.5.11".
#He is not familiar with regular expressions. Alyssa supplied Ben with a method called
#is_an_ip_number? that determines if a string is a numeric string between 0 and 255 as required
#for IP numbers and asked Ben to use it.
